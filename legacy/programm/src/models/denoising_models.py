...
Поддержка ONNX Runtime для быстрого инференса на CPU
"""
...
class LightweightDenoisingModel:
    ...
    def preprocess_image(self, image: np.ndarray) -> np.ndarray:
        ...
        if len(image.shape) == 2:
            image = np.expand_dims(image, axis=0)  # Добавляем channel
        if len(image.shape) == 3:
            image = np.expand_dims(image, axis=0)  # Добавляем batch
...
class DenoisingModelManager:
    ...
    def __init__(self):
        self.models = {}
        self.models_dir = Path("models/denoising")
...
def create_fallback_denoising():
    ...
    class FallbackDenoising:
        ...
        def wavelet_denoise(image: np.ndarray, intensity: int = 50) -> np.ndarray:
            """Вейвлет-подавление шума"""
            # Простая реализация через медианный фильтр
