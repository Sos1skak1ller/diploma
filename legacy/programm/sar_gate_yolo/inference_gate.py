...
def connected_components_from_heatmap(heat: np.ndarray, thr: float, dilate_iter: int = 1) -> List[Tuple[int, int, int, int, float]]:
    prob = 1.0 / (1.0 + np.exp(-heat))
    binm = (prob >= thr).astype(np.uint8)
    binm = cv2.dilate(binm, np.ones((3, 3), np.uint8), iterations=max(1, int(dilate_iter)))
...
def run_gate(img: np.ndarray, cfg: Dict) -> List[Tuple[int, int, int, int, float]]:
    ...
    if cfar_cfg.get("enabled", True):
        ...
        if isinstance(scales, list) and len(scales) > 0:
            for s in scales:
                s = float(s)
                small = img_proc if s == 1.0 else cv2.resize(img_proc, (int(img_proc.shape[1] * s), int(img_proc.shape[0] * s)), interpolation=cv2.INTER_AREA)
                if cfar_cfg.get("type", "OS").upper() == "OS":
                    m_small = cfar_os(small, guard=guard, train=train, q=q_val, k=k_val)
                else:
                    m_small = cfar_ca(small, guard=guard, train=train, k=k_val)
                m_full = m_small if s == 1.0 else cv2.resize(m_small, (img_proc.shape[1], img_proc.shape[0]), interpolation=cv2.INTER_NEAREST)
                masks.append(m_full)
            ...
            small = img_proc if scale == 1.0 else cv2.resize(img_proc, (int(img_proc.shape[1] * scale), int(img_proc.shape[0] * scale)), interpolation=cv2.INTER_AREA)
            if cfar_cfg.get("type", "OS").upper() == "OS":
                mask_small = cfar_os(small, guard=guard, train=train, q=q_val, k=k_val)
            else:
                mask_small = cfar_ca(small, guard=guard, train=train, k=k_val)
            mask_cfar = mask_small if scale == 1.0 else cv2.resize(mask_small, (img_proc.shape[1], img_proc.shape[0]), interpolation=cv2.INTER_NEAREST)
    else:
        mask_cfar = np.zeros_like(tiles["mask"])  # type: ignore
    ...
    if use_mininet:
        ...
        comps = connected_components_from_heatmap(heat, thr=thr, dilate_iter=dilate_iter)
    else:
        # Fallback: use Step A mask to form components
        binm = (mask_a > 0).astype(np.uint8)
        binm = cv2.dilate(binm, np.ones((3, 3), np.uint8), iterations=max(1, dilate_iter))
    ...
    def iou_xyxy(a: Tuple[int, int, int, int, float], b: Tuple[int, int, int, int, float]) -> float:
        ...
        if max(bw, bh) > max_side:
            ...
            for ty in range(y1, y2, step):
                for tx in range(x1, x2, step):
                    rois.append((tx, ty, min(tx + step, w), min(ty + step, h)))
        else:
