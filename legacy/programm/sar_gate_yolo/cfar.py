...
def cfar_ca(img: np.ndarray, guard: int, train: int, k: float) -> np.ndarray:
    ...
    mean_win = cv2.boxFilter(img32, ddepth=-1, ksize=ksize_win, normalize=True, borderType=cv2.BORDER_REFLECT)
    if g > 0:
        mean_guard = cv2.boxFilter(img32, ddepth=-1, ksize=ksize_g, normalize=True, borderType=cv2.BORDER_REFLECT)
    ...
    if abs(q - 0.5) < 1e-6:
        ...
        for y in range(h):
            for x in range(w):
                ys = y
                xs = x
                patch = padded[ys : ys + 2 * win + 1, xs : xs + 2 * win + 1].astype(np.float32)
