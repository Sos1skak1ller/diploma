...
def run_pipeline(images_path: str, config_path: str, save_vis: str = None, save_json_dir: str = None):
    ...
    for p in paths:
        ...
        rois = run_gate(img if img.ndim == 2 else cv2.cvtColor(img, cv2.COLOR_BGR2GRAY), cfg)
        ...
        all_boxes = np.concatenate(global_boxes, axis=0) if len(global_boxes) > 0 else np.zeros((0, 6))
        if all_boxes.size > 0:
            if bool(cfg["yolo"].get("use_soft_nms", False)):
                final = soft_nms(all_boxes, iou_thr=float(cfg["yolo"].get("iou_nms", 0.6)))
            else:
                final = diou_nms(all_boxes, iou_thr=float(cfg["yolo"].get("iou_nms", 0.6)))
        else:
            final = all_boxes
...
def cli():
    ...
    ap_train.add_argument("--config", default=os.path.join(os.path.dirname(__file__), "config.yaml"))
    ...
    ap_run.add_argument("--config", default=os.path.join(os.path.dirname(__file__), "config.yaml"))
    ap_run.add_argument("--save-vis")
    ap_run.add_argument("--save-json")
    ap_run.add_argument("--gate-only", action="store_true", help="Run gate only and save ROI visualization")
    ...
    if args.cmd == "train-mininet":
        ...
        sys.argv = ["train_mininet.py", "--data", args.data, "--config", args.config, "--epochs", str(args.epochs)]
        return train_main()
    if args.cmd == "quantize-mininet":
        from .quantization import main as quant_main
        import sys

        sys_argv = ["quantization.py", "--onnx", args.onnx, "--calib", args.calib, "--out", args.out]
        if args.yolo:
    ...
    if args.cmd == "convert-openvino":
        try:
            ...
            raise RuntimeError("OpenVINO is not installed. Please pip install openvino.")
    ...
    if args.cmd == "run":
        if args.gate_only:
            ...
            for p in paths:
                ...
                out_p = os.path.join(args.save_vis or "out/vis_gate", os.path.basename(p))
                cv2.imwrite(out_p, vis)
            return
        return run_pipeline(args.images, args.config, args.save_vis, args.save_json)
