...
def build_crops(img: np.ndarray, rois: List[Tuple[int, int, int, int]], target: int = 640) -> Tuple[np.ndarray, List[Dict]]:
    crops = []
    metas = []
    for (x1, y1, x2, y2) in rois:
        ...
        resized = cv2.resize(crop_c, (new_w, new_h), interpolation=cv2.INTER_LINEAR)
        canvas = np.full((target, target, 3), 114, dtype=resized.dtype)
        dx = (target - new_w) // 2
        dy = (target - new_h) // 2
        canvas[dy : dy + new_h, dx : dx + new_w] = resized
        crops.append(canvas)
        metas.append({"roi": (x1, y1, x2, y2), "scale": scale, "pad": (dx, dy), "target": target})
    if len(crops) == 0:
