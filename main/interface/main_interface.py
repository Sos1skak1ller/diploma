...
class MainInterface(QMainWindow):
    ...
    def create_enhance_control_panel(self):
        ...
        # Лёгкие CPU‑алгоритмы для SAR:
        self.enhance_type_combo.addItems([
            "Гибридное подавление шума SAR",      # bilateral + NLM
            "Адаптивное подавление спекла SAR",    # Lee/Frost‑подобный
            "Анизотропная диффузия SAR",           # Perona–Malik / SRAD
        ])
    ...
    def update_enhance_info(self, enhance_type):
        """Обновляет информацию о параметрах улучшения для выбранного алгоритма"""
        info_text = {
            "Гибридное подавление шума SAR":
                "Гибридный алгоритм подавления шума для SAR‑изображений.\n"
                "Bilateral фильтр сглаживает фон, NLM (Non‑local Means) удаляет спекл, затем результаты смешиваются.\n"
                "Подходит для общего подавления шума без потери деталей.",
            "Адаптивное подавление спекла SAR":
                "Адаптивный фильтр спекла в духе Lee/Frost‑фильтров.\n"
                "Использует локальное среднее и дисперсию, чтобы подавлять шум и сохранять границы объектов.\n"
                "Рекомендуется для сцен с выраженными структурами и контрастными границами.",
            "Анизотропная диффузия SAR":
                "Анизотропная диффузия (Perona–Malik / SRAD‑подобный алгоритм) для SAR‑снимков.\n"
                "Итеративно сглаживает однородные области и почти не размывает резкие границы.\n"
                "Подходит для мягкого сглаживания фона при максимальном сохранении контуров."
        }
