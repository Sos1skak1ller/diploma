...
# Демо-сервер снимков (Docker)
make server-up      # поднять
make server-logs    # читать логи
make server-down    # остановить
```
...
В группе **«Загрузка файлов»** на главной вкладке доступны два режима:

- **Локально** — выбор отдельных файлов (`Выбрать файлы`) или целой папки (`Выбрать папку`, рекурсивный обход всех `*.jpg/*.png/*.tif*`).
- **Сервер** — список снимков подгружается с локального демо‑сервера (см. раздел «Демо‑сервер снимков»). По двойному клику снимок скачивается в `tmp/remote/` и становится активным.

Выделение в списке = «активный» снимок: все запуски пайплайна (детекция, ROI, улучшение, полный пайплайн, SAR‑HUB) работают именно с ним. Если активный элемент не выбран, берётся первый.
...
В директории [`image_server/`](image_server/) лежит отдельный лёгкий FastAPI‑сервер, который имитирует наземную станцию / архив SAR‑снимков. GUI в режиме «Сервер» обращается к нему по REST.

### Быстрый запуск (Docker)

```bash
# 1. Положите снимки в каталог хранилища (можно использовать существующие images/)
cp images/*.jpg image_server/storage/

# 2. Поднимите сервер
make server-up      # либо: cd image_server && docker compose up -d --build

# 3. Откройте автодокументацию: http://localhost:8000/docs
make server-logs    # просмотр логов
make server-down    # остановка
```
...
1. На главной вкладке переключите радио‑кнопку источника в **«Сервер»**.
2. Введите URL (по умолчанию `http://localhost:8000`) и нажмите **«Обновить список»**.
3. В списке появятся снимки с сервера. Двойной клик по элементу — скачивание в `tmp/remote/` и установка активным.
4. Дальше всё как обычно: детекция, улучшение, ROI, классификация SAR‑HUB.

Подробнее об эндпоинтах и переменных окружения — в [`image_server/README.md`](image_server/README.md).
